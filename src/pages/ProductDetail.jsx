...
import { ShoppingCart, ArrowLeft, ShieldCheck, Truck, Loader2, Star, MessageSquare, Send } from 'lucide-react';
...
const ProductDetail = () => {
  ...
  if (error || !product) return <div style={{ padding: '5rem', textAlign: 'center' }}><h2>{error || 'Producto no encontrado'}</h2><Link to="/productos" style={{ color: '#3b82f6' }}>Volver al catálogo</Link></div>;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#475569', backgroundColor: '#f8fafc', padding: '1.2rem', borderRadius: '20px' }}>
              ...
              <div>
                <p style={{ margin: 0, fontWeight: 'bold', fontSize: '0.9rem' }}>Envío Gratis</p>
                <p style={{ margin: 0, fontSize: '0.8rem', color: '#94a3b8' }}>Llega en 3-5 días</p>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#475569', backgroundColor: '#f8fafc', padding: '1.2rem', borderRadius: '20px' }}>
              <ShieldCheck size={24} color="#3b82f6" /> 
              <div>
                <p style={{ margin: 0, fontWeight: 'bold', fontSize: '0.9rem' }}>Garantía Tech</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: '5rem' }}>
        ...
        <div>
          <h3 style={{ fontSize: '1.8rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <MessageSquare size={24} color="#3b82f6" /> Danos tu opinión
          </h3>
          ...
          {user ? (
            <form onSubmit={handleReviewSubmit} style={{ backgroundColor: '#f8fafc', padding: '2rem', borderRadius: '24px', border: '1px solid #e2e8f0' }}>
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '10px', fontWeight: 'bold', color: '#64748b' }}>Calificación:</label>
              ...
              >
                {submitting ? <Loader2 className="animate-spin" size={20} /> : <><Send size={18} /> Publicar Reseña</>}
              </button>
            </form>
          ) : (
            <div style={{ backgroundColor: '#eff6ff', padding: '2rem', borderRadius: '24px', border: '1px solid #bfdbfe', textAlign: 'center' }}>
              <p style={{ color: '#1e40af', fontWeight: '600', marginBottom: '1rem' }}>Inicia sesión para compartir tu experiencia con otros compradores.</p>
        ...
        <div>
          ...
          {reviews.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '4rem', color: '#94a3b8' }}>
              ...
              <p>Aún no hay reseñas para este producto. ¡Sé el primero en calificarlo!</p>
            </div>
          ) : (
